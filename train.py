...
def train(args):
    ...
    # 修改原因：修改数据集读取格式
    train_set = APIDataset("data/train.feather", config['max_sent_len'])
    valid_set = APIDataset("data/test.feather", config['max_sent_len'])
    ...
    scheduler = get_cosine_schedule_with_warmup(
        ...
        num_training_steps=len(train_loader) * config[
            'epochs'])  # do not foget to modify the number when dataset is changed
    ...
    for epoch in range(start_epoch, config['epochs'] + 1):
        ...
        save_model(model, epoch, timestamp)

        model.eval()
        valid_loader = torch.utils.data.DataLoader(dataset=valid_set, batch_size=1, shuffle=False, num_workers=1)
        vocab_api = load_dict("data/vocab.apiseq.json")
        vocab_desc = load_dict("data/vocab.desc.json")
        file_path = f"./output/{args.model}/{args.expname}/{timestamp}/models/{epoch}"
        test.evaluate(model, valid_loader, vocab_desc, vocab_api, "beamsearch", 10, file_path)

    save_model(model, "final", timestamp)  # save model after each epoch
